/* Neste arquivo são apresentados exemplos de uso dos comandos BEGIN, COMMIT e ROLLBACK da linguagem de operação DTL (Linguagem de Transação de Dados) da PL/SQL.

PL/SQL (Program Language SQL) é a linguagem de programação da Oracle que objetiva efetuar transações de informações do banco de dados
    • Cria variáveis herdando o tipo e tamanho de outras variáveis ou objetos do banco de dados tais como tabelas;
    • Utiliza comandos de repetição e comparação;
    • Trata erros;
    • Cria cursores para tratamento de resultado de pesquisas e cria registros para armazenar resultados de cursores;
...
BEGIN /*Operações SQL e PL/SQL (obrigatório)*/
    ...
    /*'DBMS_OUTPUT' métodos para imprimir ou gravar no buffer, '.PUT_LINE ' grava o parâmetro no buffer*/
END; /*(obrigatório)*/
